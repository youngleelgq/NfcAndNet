...
public class TransportUtils {
	...
	public static byte[] intToBytes( int n ) {
		return new byte[] { (byte) ( 0x000000ff & ( n >>> 24 ) ), (byte) ( 0x000000ff & ( n >>> 16 ) ), (byte) ( 0x000000ff & ( n >>> 8 ) ), (byte) ( 0x000000ff & ( n ) ) };
	}
	...
	public static String fen2Yuan(String fen ) {
		float yuan = Float.valueOf( fen ) / 100.0f;
		return String.format( "%.2f", yuan );
	}

	// 分转元 不用格式化
	public static String fen2Yuan_(String fen ) {
		int yuan = (int) ( Integer.valueOf( fen ) / 100.0 );
	 ...
	 * @return
	 */
	public static String SubStringMessage(String msg ) {
		int index = msg.indexOf( ':' );
		return ( index != -1 ? msg.substring( index + 1, msg.length( ) ) : msg ).trim( );
	}
	...
	public static String getRandom( ) {
		...
		for( int i = 0; i < 32; i++ ) {
			randomStr.append( random.nextInt( 9 ) );
		}
