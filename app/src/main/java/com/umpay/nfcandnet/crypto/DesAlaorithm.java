...
public class DesAlaorithm {
     ...
     * DES解密--->对称密钥
     * <p>
     * 解密算法与加密算法基本相同，不同之处仅在于轮子密钥的使用顺序逆序
     * <p>
     * 轮子密钥为加密的轮子密钥，解密的轮子密钥为加密的轮子密钥，
     * <p>
     * 解密的第16 轮子密钥为加密的轮子密钥
     *
     * @param source 密文
     ...
     * 16进制的字符串转换成ASC的字符串
     * <p>
     * 16进制的字符串压缩成BCD编码 30313233343536373839414243444546)-->(0123456789ABCDEF)
     *
     * @param hex
     * @return
     */
     ...
     * @return
     */
    public static String MAC3(String key, String data, String ICV) {

        if (data.length() % 16 != 0) {
            throw new IllegalArgumentException();
        }

        String keyleft = key.substring(0, 16);
        String keyRight = key.substring(16);
        ...
        for (int i = 0; i < data.length() / 16; i++) {
            data_ = data.substring(i * 16, (i + 1) * 16);
            data_ = xOr(data_, vector);
            vector = DES_1(data_, keyleft, 0);

        }

        data_ = DES_1(vector, keyRight, 1);
        data_ = DES_1(data_, keyleft, 0);
     ...
     * @return
     */
    public static String DES3_ECB_ENC(String key, String data) {
     ...
     * @return
     */
    public static String DES3_ECB_DSC(String key, String data) {
    ...
    public static byte[] symmetricDecrypto(byte[] byteSource, String sKey) throws Exception {
        try {
            ...
            Cipher cipher = Cipher.getInstance("DES");
            cipher.init(mode, key);
            LogUtils.e("symmetricDecrypto需要解密的数据:" + Utils.bytesToHexString(byteSource));
            byte[] result = cipher.doFinal(byteSource);
            return result;
        } catch (Exception e) {
            throw e;
